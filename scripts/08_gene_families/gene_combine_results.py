...
"""
===============================================================================
STEP 3: COMBINE GFF KEYWORD + HMMER RESULTS INTO GENE-FAMILY MASTER LIST
===============================================================================

# CHANGED: 2026-03-31 — Renamed from cyp_combine_results.py → gene_combine_results.py.
#   Added --family-name arg for display labels. Works for CYP, OMT, or any family.

Merge the two gene-family candidate lists from Step 1 (GFF keyword search) and
...
Each gene is flagged with its evidence source:
  ...
  - "HMMER_only" → has domain but no keyword match (may have unusual annotation)
  - "GTF_only"   → keyword match but no domain hit (may be pseudogene, fragment,
                    or false positive from keyword matching)

Input files:
  - <family>_gtf_candidates.csv   (from Step 1)
  - <family>_hmmer_confirmed.csv  (from Step 2)

Output:
  - <family>_master_list.csv
...
USAGE:
  # CYP:
  python scripts/08_gene_families/gene_combine_results.py \
  ...
  # OMT:
  python scripts/08_gene_families/gene_combine_results.py \
      --gtf-csv omt_gtf_candidates.csv \
      --hmmer-csv omt_hmmer_confirmed.csv \
      --output omt_master_list.csv \
      --family-name OMT
...
def main():
    # CHANGED: 2026-03-31 — Added --family-name for generic label in output
    parser = argparse.ArgumentParser(
        description='Step 3: Combine GFF keyword + HMMER results into '
                    'a gene-family master list',
    ...
    parser.add_argument(
        '--family-name', default='CYP450',
        help='Display label for this gene family (default: CYP450)',
    )
    args = parser.parse_args()

    print("=" * 70)
    print(f"{args.family_name} DATABASE — STEP 3: COMBINE RESULTS")
    print("=" * 70)
